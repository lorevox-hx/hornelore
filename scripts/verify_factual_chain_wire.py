...
def verify_db_column() -> Tuple[bool, str]:
    ...
    Path mirrors server/code/api/db.py: DATA_DIR/db/DB_NAME (the
    server keeps sqlite under a 'db' subdir, not the top-level
    DATA_DIR). Defaults match the .env: DATA_DIR=/mnt/c/hornelore_data
    and DB_NAME=hornelore.sqlite3.
    """
    data_dir = os.getenv("DATA_DIR", "/mnt/c/hornelore_data")
    db_name = (os.getenv("DB_NAME", "hornelore.sqlite3").strip()
               or "hornelore.sqlite3")
    db_path = Path(data_dir) / "db" / db_name
