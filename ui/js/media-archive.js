...
(function () {
  ...
  // served from port 8082 (hornelore-serve.py); API lives on port 8000.
        ...
        .then(function (r) {
          ...
          if (r.status === 404) {
            throw new Error(
              ...
              "Set HORNELORE_MEDIA_ARCHIVE_ENABLED=1 in .env and cycle the stack."
            );
          }
          if (!r.ok) {
  ...
  function refreshList() {
    ...
    fetch(url)
      .then(function (r) {
        if (r.status === 404) {
          throw new Error("Archive surface disabled (HORNELORE_MEDIA_ARCHIVE_ENABLED=0).");
        }
  ...
  fetch(ORIGIN + "/api/media-archive/health")
    ...
    .then(function (h) {
      if (!h || !h.enabled) {
        setStatus(
          "Document Archive is DISABLED. Set HORNELORE_MEDIA_ARCHIVE_ENABLED=1 " +
