...
(function () {
  ...
  async function lvExportCurrentSessionArchive() {
    ...
    const enabled = await _probeEnabled();
    if (!enabled) { _stat("⚠ archive disabled (HORNELORE_ARCHIVE_ENABLED off?) — check api health"); return false; }
    ...
    let filename = "hornelore_archive_" + personId.slice(0, 8) + "_" +
  ...
  console.log("[Hornelore] archive-writer loaded.");
})();
