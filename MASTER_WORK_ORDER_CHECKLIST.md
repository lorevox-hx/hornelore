...
**As of:** 2026-05-03 (Voice Library v1 + Narrative Cue Library v2 + WO-DISCLOSURE-MODE-01 spec day, post r5h-followup-guard-v1 baseline lock)
...
**Parent sessions:** Day-of-readiness — Reset Identity utility staged, scan_answer hardening landed, BUG-DBLOCK-01 closed (write-rollback hygiene mirrored across `save_safety_event` + 4 `story_candidate_*` writers). Lori-behavior reflection lane: BUG-LORI-REFLECTION-01 base committed (`ea13cbf`), Patch B REJECTED + reverted (golfball regressed 4/8 → 1/8 — locked principle: prompt-heavy reflection rules make Lori worse; runtime shaping is the path). Clock UI v1 landed (`2e10aba`). **2026-05-02:** WO-EX-UTTERANCE-FRAME-01 promoted from PARKED to ACTIVE Phase 0-2 — pure-deterministic Story Clause Map builder + 24 fixtures + 16 unit tests + 4-test build-gate isolation + CLI debug runner + chat_ws observability log behind `HORNELORE_UTTERANCE_FRAME_LOG=1` (default-off). 24/24 tests green; consumer wiring deferred to Phase 3+. **2026-05-02 evening:** WO-EX-CASE-BANK-FIXUP-01 Phase 1 shipped (case_110 spouse path normalized bare `spouse.*` → canonical `family.spouse.*`); BUG-EX-BIRTH-DATE-PATTERN-01 + WO-EX-CASE-BANK-FIXUP-01 umbrella specs parked. **Today (2026-05-03):** Voice Library v1 + Narrative Cue Library v2 + Cultural Humility Eval Pack + WO-DISCLOSURE-MODE-01 + WO-LORI-SENTENCE-DIAGRAM-RESPONSE-01 banked. r5h-followup-guard-v1 master eval verified case_110 flip (78/114, +1 across all three contract metrics, mnw stable, scorer-drift ZERO). New active baseline locked.
