...
## Post-unification epic -- WO-TRAVEL-DOC-IMPORT-PROVENANCE-FOUNDATION-01 (🟡 **Phase 0 + Phase 1 done 2026-07-26; Phase 2+ BLOCKED on two operator decisions**)
...
| 1.1 -- resolve the two Christopher rows | ⛔ **BLOCKED on Chris.** Merge onto `a4b2f07a`, keep them deliberately separate, or add a `merged_into` canonical pointer (reversible; the recommended shape if a merge is wanted). **This blocks the first import, not the migration.** Whatever lands, the Phase 1 script is the before/after instrument and should be re-run on both sides of it. |
| 2 -- migration 0037 | ⛔ **BLOCKED.** `import_batch` + `import_candidate` per the Epic Plan, `hidden`/`hidden_at` following `0036`'s reversible-hide precedent, keyed per the A/B/C decision. |
| 3 -- repository layer | Not started. `services/import_repository.py` + unit suite. Import is intake, not approval. |
| 4 -- minimal verification surface | Not started. Operator-only list/create/decide -- the least UI that lets Phase 5 happen. Not the Evidence Review Queue; that is WO-2. |
| 5 -- live smoke | Not started. |

**The two decisions, in the order they matter:**

1. **The two Christopher rows.** `a4b2f07a` holds the trips, the photo sessions
   and the older library; `e7fdb578` holds the bio_facts and every photo added
   since 2026-07-25. Import Provenance writes a durable `person_id` onto every
   candidate row it creates, so provenance recorded against the wrong
   Christopher is worse than none -- it looks correct.
2. **A / B / C for `import_candidate`'s identity key.** A = match the status quo
   (`person_id` with an FK to `people`, fault line documented but unenforced).
   **B = A plus the missing FKs on `photos.narrator_id` and
   `trip_photo_links.photo_id`** -- now measured safe, and recommended. C =
   defer, which means the first thing the epic builds is the first thing that
   needs migrating again.
