...
| 11b | **`WO-LORI-CONVERSATION-TO-LIFE-MAP-MEMOIR-01`** | ✅ **ACCEPTED AND COMPLETE 2026-08-20** | Story-to-memoir synthetic chain **11/11**; deletion-integrity acceptance **10/10**. Every claim verified against the **filesystem and direct SQL**, never a response body. Conversation → capture → extraction → operator review → chronology/Life Map → preview/TXT/DOCX, the story appearing **exactly once** in all three with one provenance digest. Migrations 0047–0050. Spec: [`docs/wo/WO-LORI-CONVERSATION-TO-LIFE-MAP-MEMOIR-01_Spec.md`](docs/wo/WO-LORI-CONVERSATION-TO-LIFE-MAP-MEMOIR-01_Spec.md). All synthetic narrators and acceptance residue removed; **the five family narrators are untouched**; `integrity_check ok`; the **six pre-existing `harness-test-gate7p2` FK violations are unchanged and NOT closed here**. Gate B **OPEN**, L2 **PARTIAL**, directive registry **inert**, Kawa only as frozen legacy UI plus one erasure-inventory directory. |
| 11c | **Deletion integrity** | ✅ **CLOSED 2026-08-20 — a privacy defect the lane's own cleanup exposed** | `hard_delete_person` removed every row, answered 200, and left eight files on disk, five of them verbatim narrator speech. Erasure now plans before the database authority is destroyed and persists that plan bound to the canonical absolute root; refuses every symlink below the root, including one pointing at ANOTHER narrator inside it; covers eleven stores; **deletes media rather than detaching it** (Chris, 2026-08-20 — `ON DELETE SET NULL` stays as a database fallback, but a confirmed hard erasure must not leave identifiable photographs as ownerless rows); purges the translation cache; **reports backups and exports rather than rewriting them**; fails closed before touching a row; and is retryable through the product API with a truthful audit trail. |
