...
| 6 | **Remove the retired Kawa / Memory River implementation** | **Removal DECIDED by Chris on 2026-09-08** — the adjudication step is gone, not deferred. Bounded follow-up work order: unmount the router (`server/code/api/main.py:187`), remove `routers/kawa.py`, `kawa_store.py`, `kawa_projection.py`, `ui/js/lori-kawa.js`, the Kawa client helpers in `ui/js/api.js`, the state/preload/render/**memoir-context** paths in `ui/js/app.js`, the button/popover/CSS/mode selectors in `ui/hornelore1.0.html`, `data/prompts/kawa_prompts.json`, and the `chronology_river` mode **after checking whether any persisted setting needs a safe fallback**. Add a negative structural test proving the product can no longer expose or route Kawa. **Keep** historical research and archived Kawa specs. **Do not delete narrator data under `DATA_DIR/kawa/`** — stop creating and reading it first; deletion is a separate data-retention decision. *(This row read "ADJUDICATE removal…" and said removal was not decided. It was not, until 2026-09-08. It is now.)* |
