...
| # | Work | State |
|---:|---|---|
| 1 | **`WO-LORI-ARCHIVE-TO-MEMOIR-02`** — Phase 4 | 🔵 **CURRENT — PHASE 4.** Phases 0–3 CLOSED. **Phase 3 ACCEPTED 2026-09-05**: all three meaning-corruption defects fixed — (a) the empty-parse correction fallthrough resets BOTH copies of `turn_mode`, (b) the group-local kinship guard quarantines an unstated relationship whole-group and entity-bound, (c) **per-value grounding** annotates `spoken`/`derived`/`unsupported` per value, never per group. Carriers landed too: review-only results travel end to end, and the server's authority decision now binds the browser (it did not before, which also silently affected shipped transcript safety). **The gate closed live**, not on source reading: Stefi correction-fallthrough `20260905-021741`, **9/9 over the production WebSocket** — no `correction_payload`, no `correction-apply`, and a story candidate preserved AND bound, which a deterministic turn never does. Clean finals: `r5k-guard-v2` 71/114 at `5afead5` and `r5k-generational` 7/14 at `4ab00fc`, both clean trees, both 0 `must_not_write`. **The extraction baseline is NOT comparable** — r5h/r5j/r5k span three scorers and two dirty trees; the `2 → 0` `must_not_write` delta is unestablished (r5k's **0** is solid current evidence). Nonblocking debt: [`docs/BACKLOG.md`](docs/BACKLOG.md) §6a. **PHASE 4 DESIGN SETTLED 2026-09-05: persist the already-computed capture decision directly on the COMMITTED NARRATOR TURN — one implementation checkpoint, then one live-acceptance/closeout checkpoint.** The turn, not a new table: it avoids a new table, inherits narrator deletion automatically, and covers declined turns, which have no candidate to attach to. **NOT in scope:** another classifier rewrite · another 128-case eval · an extraction-ledger repair · any capture-threshold change. The decision is already recorded for all 38 turns at `chat_ws.py:1848`, into a gitignored rotating log — **not durable is not absent**; presence is 35/38 byte-exact with zero over-capture. Phase 4 prose predating this is marked SUPERSEDED, and its "persisted nowhere" / "27 no-candidate turns" / ledger-repair claims are disproven. **One Phase 3 box stays unchecked — *Jim binds as Pat's husband*: `Otis` is in five test suites, `Domingo` in two, `Jim` in none**; Pat's Jim passage is Phase 4's primary regression fixture. Phase 2's broader correctness obligations remain **TRANSFERRED to Phases 5/6/8**. Reproduce the Phase 2 audit: `python3 scripts/phase2_verify_ledger.py`. State: `HANDOFF.md` §9. [Work order](docs/wo/WO-LORI-ARCHIVE-TO-MEMOIR-02.md) |
...
| 3 | **`WO-REPOSITORY-HYGIENE-01`** — indexed reorganization | ⏸️ **PHASE A ACCEPTED, REMAINDER PAUSED — INCOMPLETE.** Steps 0, 1, 2, 2b and the first Step 3 cohort accepted. **Deferred by Chris's product-priority decision:** the remaining Step 3 cohorts, Steps 4–5, and the final verification checkpoint. Hashes: `HANDOFF.md` §1. [Spec](docs/wo/WO-REPOSITORY-HYGIENE-01_Spec.md) |
