...
| # | Work | State |
|---:|---|---|
| 1 | **`WO-LORI-ARCHIVE-TO-MEMOIR-02`** — Phase 3 | 🔵 **CURRENT — IMPLEMENTATION COMPLETE, ONE ACCEPTANCE GATE OPEN.** Phases 0–2 CLOSED. **Both meaning-corruption defects are fixed**, and a third found alongside: (a) the empty-parse correction fallthrough resets BOTH copies of `turn_mode` so Stefi's clarification gets ordinary interview processing; (b) the group-local kinship guard quarantines a relationship the narrator did not state, whole-group, entity-bound; (c) **per-value grounding** — `parents.birthDate=1922` was fabricated, and grounding is annotated per value (`spoken`/`derived`/`unsupported`), never per group. Supporting carriers also landed: review-only results travel end to end, the server's authority decision now binds the browser (it previously did not — this also silently affected shipped transcript safety), and confirmation reasons are additive. **THE OPEN GATE: the corrected 128-case live evaluation** (`master_loop01_r5j-phase3-v1`) — read `preservation_fates`, not the pass count alone, because a quarantined value is now preserved rather than lost. Phase 2's broader correctness obligations remain **TRANSFERRED to Phases 5/6/8**. Reproduce the Phase 2 audit: `python3 scripts/phase2_verify_ledger.py`. State: `HANDOFF.md` §9. [Work order](docs/wo/WO-LORI-ARCHIVE-TO-MEMOIR-02.md) |
