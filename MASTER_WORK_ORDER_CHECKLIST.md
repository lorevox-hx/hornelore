...
| # | Work | State |
|---:|---|---|
| 1 | **`WO-LORI-ARCHIVE-TO-MEMOIR-02`** — Phase 3 | 🔵 **CURRENT.** **Phases 0, 1 and 2 are CLOSED.** Phase 3 fixes the two defects that corrupt MEANING: a clarification can be swallowed by the correction route (`chat_ws.py:4341` returns unconditionally even when `parsed == {}`), and spouse names can be proposed under `parents.*`. **No mutation is authorized and no live run is pending.** Phase 2's broader correctness obligations were **TRANSFERRED to Phases 3/5/6/8, not completed** — see its closeout. Reproduce the Phase 2 audit: `python3 scripts/phase2_verify_ledger.py`. State: `HANDOFF.md` §9. [Work order](docs/wo/WO-LORI-ARCHIVE-TO-MEMOIR-02.md) |
