...
**Active as of:** 2026-08-18 (Story Integration COMPLETE — Phases 1–4 accepted; next lane is finishing Lean Lori)
...
| 12 | **Finish Lean Lori — NEXT LANE, one substantial implementation block** | 🔵 **NEXT — not started** | Six items in order, detailed in `HANDOFF.md` §4: (1) complete prompt-section metadata — owner, activation condition, trim policy, source, priority tier, real token count, redacted hash; (2) finish directive gating so each family appears only when its state/feature/task is active (**Profile Seed DECIDED — Option A, live narrators only**); (3) **decide history-versus-optional-section priority FROM MEASUREMENTS** — Phase 4 exhausts history first, which preserved every working turn, and its telemetry now emits the per-section token costs that decision needs; (4) finish passive diagnostics — one operator-readable record across REST chat, REST streaming and WebSocket, no narrator prose; (5) a SMALL live acceptance replacing the abandoned L2 campaign — ordinary conversation, one state-heavy turn, one trip turn, one approved-story turn, one safe oversized refusal; (6) reconcile the Lean Lori WO's stale status table and remove its abandoned runtime-profile rollback language and unsatisfiable Gate F assumptions. |
