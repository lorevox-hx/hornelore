...
5. **Smoke-photo residue** -- narrator `e7fdb578`'s library now holds **six** disposable smoke photos (photo *links* are deleted with a trip, photos are not). Needs either a cleanup pass or a documented "these are test rows".
...
**Also open, and worth deciding first:** neither Playwright liveness harness runs on Chris's machine, because `node_modules` does not exist in the working tree. They were run in the cloud container after md5 proved the relevant files byte-identical, which is sound, but it means the harness half of the proof chain is not reproducible where the code lives. Either `npm install`, or document that these two do not run there.
