...
| # | Work | State |
|---:|---|---|
| 0 | **`WO-LOREVOX-PORTABLE-NARRATOR-01`** — Portable Narrator | ✅ **PACKAGE PORTABILITY COMPLETE 2026-09-14 — all six source packages proven; hands off to Merge/Remap (row 0a).** **Laptop Christopher RE-CLOSED (§36.5a):** the reopening was a stronger validator, not a failed proof — 9 `turn_extraction_ledger` rows carried encoded `turnrow:<id>` references to turns the package did not contain. **Stale derived bookkeeping and only that.** Repair on the authoritative laptop DB: ids **7,8,9,10,11,17,18,19,28** removed and nothing else, one `BEGIN IMMEDIATE` requiring `rowcount`=9 with every surviving row fingerprint-compared, originals written to a recovery artifact first. **533 → 524 records, files unchanged at 214**, DB sha256 `1cd55ec6…`. A later `--apply` correctly refused at 0/9 — the safety check working. New authoritative package **`24560db21dd6`** (superseding `a2f360689b58`) restored into a fresh root, **narrator never opened**, re-exported as `f03759af038a`: **`EQUIVALENT tables=60 rows=524 files=214`**, **zero unresolved FK / `ColumnRef` / encoded-TEXT / encoded-JSON references**. **Nothing else changed** — no ownership materialised, no `trip_turn_links` touched, no curation — so this re-proves the same `DirectOrExclusiveInbound` closure §36.5 proved. **Christopher has ZERO directly-owned sessions**, making that closure load-bearing for a real family narrator. **The preservation/curation analysis is Phase 7 input and gates nothing.** *(Previously: "TWO-ORIGIN COMPARISON COMPLETE (evidence phase); ONE PHASE 6 PACKAGE REOPENED.")* **Prior state, retained:** All three pairs compared read-only 2026-09-12, six packages byte-identical before and after. **The comparison did its job: it found a defect the Phase 4 and Phase 6 proofs structurally could not see.** `turns.id` is stored in SEVEN places and the exporter's §30 checker covered TWO, so the laptop's Christopher package carries **9 dangling** `turnrow:<turns.id>` ledger references — structurally valid, semantically incomplete under the new invariant. **Desktop proof stands for all three; laptop proof stands for Kent and Janice under the expanded audit; laptop Christopher must be investigated and re-exported before it is a merge input.** Repair landed the same day: `ENCODED_REFERENCES` in the declaration (TEXT + JSON forms), two new `ColumnRef`s for `story_candidates`' 0047 turn columns, exporter consumes the declaration, malformed encodings refuse. **Distinguishing an ownership gap from stale ledger rows is DONE — laptop investigation closed and committed 2026-09-13** (`09044ed` · `b64bc85`; findings + dry-run repair plan in `docs/wo/..._SESSION-OWNERSHIP-FINDINGS-2026-09-13.md` and `..._CHRISTOPHER-REPAIR-PLAN-2026-09-13.md`). **It was an ownership gap, and the nine references were the symptom of three whole NULL-owner `sessions` / 28 turns.** Ownership settled structurally (one `interview_sessions` row each, naming Christopher — 0044's strongest recorded link). **Runner exonerated; `applied_at` proves only that nothing raised, never that an UPDATE matched** — 0044 filled at most 1 of its 81 candidates and **the exact failure mechanism is not recovered and is not claimed**. **Del is not an active writer regression** (pre-deployment-indeterminate). Of 912 ownerless sessions 81 are attributable, and they are **not 81 lost conversations**: 36 are synthetic personas, most of Christopher's 42 are travel-doc/guard/safety development archaeology. **Janice and Kent are NOT rebuilt** — hers has zero narrator-authored speech, his is a duplicated harness fixture. **Net salvage: one genuine family fact plus one photo correction.** **Still owed for Christopher:** Chris's decisions on the four §1b candidates and the §4 ledger-reference choice (plan argues declare `turn_extraction_ledger → turns` **and** remove the stale rows, rather than importing 28 development turns), then a narrow ownership repair **on the laptop**, a new authoritative package, and a rerun of his proof and two-origin comparison only. **Merge/Remap design continues; implementation waits.** *(Previously: "Phase 6 passed on both origins; the two-origin comparison is the current action.")* Desktop 2026-09-11 (§36.1–36.3) and laptop 2026-09-12 (§36.5): Christopher, Kent and Janice each restored into their own clean root and re-exported `EQUIVALENT` with no narrator interaction, on both machines. The laptop found the **ownership-closure** defect the desktop data could not reach (20 `trip_turn_links` → 9 residue `sessions` / 100 turns; new `DirectOrExclusiveInbound` owner type, derived never written, export/erasure parity, closure resolved before deletion; 109 rows brought under the rule; residue suite 18/18, five-suite bank 90/90). *(This cell said "six packages exist on two machines and the comparison needs both sets in one place — a transfer nobody has run" while its own opening line recorded all three pairs as compared on 2026-09-12. Both could not be true; the transfer happened and the comparison ran. **Corrected 2026-09-14.** The sentence was written before the comparison and never revisited — the stale-status defect this file exists to avoid.)* Its output is the input to the Multi-Origin Merge/Remap WO (row 0a), which is designed after it, proven on synthetic two-origin data, and rehearsed into a new root before Phase 7. Phase 5a accepted live 2026-09-11 (§35.3). *(This cell said "laptop origin next" until 2026-09-12 — it had already passed.)* Narrator Data Center on the Operator tab (`HORNELORE_OPERATOR_PORTABLE_NARRATOR=1`): router `operator_narrator_package.py`, migration `0056` export-job ledger, two-verdict import (integrity / readiness), server stepper, restore only while READY, no verb deletes narrator data; API suite `tests/test_operator_narrator_package_api.py` under `.venv`. **Phase 5b (rich per-domain inspection) is post-Phase-7 and blocks nothing.** Phases 2–4 LANDED 2026-09-10/11 (WO §32 · §33 + §33.1 · §34) — one service `narrator_package.py` (export / validate / dry-run / restore / recover / compare), CLI, migrations `0054` + `0055`, `bagit==1.8.1`; last full combined acceptance under `.venv` **66/66** (restore 18, round-trip 9, export 24, parity 15); post-review focused additions restore 19/19 and round-trip 10/10, not rerun combined. Synthetic Ada with the full travel domain exports, restores into an empty root with ids verbatim, survives simulated process death at both crash windows, and round-trips semantically equivalent. **Known V1 boundary:** installation-local integer ids (`turns.id`) — same-origin packages coexist, multi-origin may collide and V1 refuses; Merge/Remap is a separate WO owed before a one-root family cutover. |
| 0a | **`WO-LOREVOX-MULTI-ORIGIN-MERGE-REMAP-01`** — combining one narrator from two installations | 📝 **DESIGN DRAFT 2026-09-12, not approved, nothing implemented** — [spec](docs/wo/WO-LOREVOX-MULTI-ORIGIN-MERGE-REMAP-01_Spec.md). Still **OWED before the Phase 7 one-root cutover**. The `turns.id` reference inventory it was waiting on is **done and is the finding that shapes it: the id is stored in SEVEN places and a `PRAGMA foreign_key_list` walk finds TWO** — two declared integer columns, two undeclared `story_candidates` integer columns (0047, no SQL FK), two `turnrow:<id>` TEXT keys (0038/0041) and a `bio_facts.source` JSON field (`bio_fact_router.py:367`). So **a merged database can pass `foreign_key_check` and still be broken**, and the WO owes a semantic reference validator. `turns` has no defensible cross-origin key (no stable per-conversation ordinal) and is merged by conversation, never paired. V1 refuses every conflict rather than resolving it. **Melanie is out of scope** by Chris's decision 2026-09-12. Not absorbed into Portable Narrator V1. | Phase 0 landed (`20260910T015024Z`); Phase 1 landed 2026-09-10 (ownership declaration `narrator_data_inventory.py`, three erasure repairs, gap 4/4 + parity 15/15 under `.venv`; WO §31). **Two-machine data audit CLOSED 2026-09-10** — eleven labelled reports and `scripts/phase0_report_comparator.py`, all under `.runtime/`; family ids identical on both machines, each narrator's lanes split across them, neither machine is "the copy"; Phase 6 exports from both. **Phase 2 is governed by WO §30**: any narrator, the declaration as the only ownership authority, the complete travel domain generically whenever present, ids preserved, referential integrity refused-never-dangled, no deletion, no merge; proven on synthetic Ada with a seeded travel domain before any real narrator. Decided 2026-09-09: portability lands before more is built on Hornelore. [Work order](docs/wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md) |
