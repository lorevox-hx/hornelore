...
Flag gate: HORNELORE_ARCHIVE_ENABLED (default off).  When off, every
handler returns 404 via ``_require_enabled``.  Storage cap via
HORNELORE_ARCHIVE_MAX_MB_PER_PERSON (default 500) + HORNELORE_ARCHIVE_WARN_AT
...
log = logging.getLogger("hornelore.memory_archive")
...
def _require_enabled() -> None:
    """Return 404 when HORNELORE_ARCHIVE_ENABLED is off."""
...
def _max_mb_per_person() -> int:
    ...
    raw = os.environ.get("HORNELORE_ARCHIVE_MAX_MB_PER_PERSON", "500")
    try:
        val = int(float(raw))
...
def _warn_at_fraction() -> float:
    ...
    raw = os.environ.get("HORNELORE_ARCHIVE_WARN_AT", "0.8")
    try:
        val = float(raw)
        if 0 < val < 1:
            return val
...
def export_person(person_id: str) -> StreamingResponse:
    ...
    filename = f"hornelore_archive_{pid}_{ts}.zip"
