...
> | **Guard Lab — what is NOT built yet** | The LIVE acceptance that needs a running stack — no-restart toggling, the mid-turn freeze observed rather than only structurally proven, the stale-client clamp against a real second tab, and restart persistence. Then the lean baseline prompt, a populated synthetic Walt, and the owed §5 short-natural run. **No Walt/John run until `All Switchable Off` is truthful across all four surfaces** |
