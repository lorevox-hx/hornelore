...
> | **C-2 status** | **R7 live acceptance PASSED 2026-09-24** (after the draft-0063 repair); closes once the focused `.venv` gate passes and the repair is committed (checkpoint "C-2 — LIVE ACCEPTANCE FAILED"). Do NOT start C-3 |
> | **next** | one-time `lr_*` repair on the working root (draft 0063 was applied live) → restart → live smoke R7 → C-2b legacy-surface cleanup → C-3 | Brief reviewed against the tree: checkpoint "Batch C — review". One gap, recoverable Remove, is designed there as a removal state (`0065`, C-6). The next stack start is the first Phase 3 product session |
>
