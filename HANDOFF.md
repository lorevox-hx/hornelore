...
> | **Guard Lab — what is NOT built yet** | A compact Guard Lab card on the main Operator tab: current configuration (Defaults / Lean / Custom), revision, whether **the narrator currently being interviewed** is eligible, armed + recording state, the two atomic buttons, and a link into the Bug Panel. **The 43-row table is NOT duplicated there.** This closes the one gap the live run exposed: the panel knows an eligible narrator EXISTS, not whether you are talking to them. Then the lean baseline prompt and a **separate populated** synthetic narrator — the empty one stays the instrumentation narrator — and the owed §5 short-natural run |
