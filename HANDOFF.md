...
> # ▶ CURRENT ACTION — **INTEGRATED LIFE RECORD: BATCH C — C-3 CLOSED; C-4 (DATES, PLACES, EVENTS AND PERIODS) ACTIVE (2026-09-24)**
>
> The NOW block is directly below this heading. **It is the authority for what to do next.**
>
> *(This heading read "PHASE 1 CLOSED (B3 ACCEPTED); PHASE 2 / BATCH B — BUILD PLAN" and the NOW line below "Next: Batch C … starting at C-1" until 2026-09-24 — while the table under them already said C-2b CLOSED. **A third instance of the heading disagreeing with its body**, found in external review of `16a68e1`. Both moved in this edit.) (It read "THE LAPTOP REBUILD. PHASE 7 IS ACCEPTED AND CLOSED" until 2026-09-22, "THE TWO-ORIGIN COMPARISON" until 2026-09-14, and "PHASE 5 (operator jobs + UI)" before that. **Twice the heading has disagreed with its own body**, the two-truths-in-one-file drift `CLAUDE.md` opens by naming. The rule that keeps being broken: when the NOW block moves, the HEADING moves in the SAME edit. This edit moves both.)*
>
> ---
>
> # NOW — **INTEGRATED LIFE RECORD. Batch C: C-1, C-2, C-2b, C-3 CLOSED. C-4 — dates, places, events and periods — ACTIVE; it opens with the bounded-EDTF date-value decision, before any parser or migration.** `WO-HORNELORE-INTEGRATED-LIFE-RECORD-01` · the ordered roadmap is row ★ of `MASTER_WORK_ORDER_CHECKLIST.md`
>
...
> | **Batch C** | **C-1 closed** (`e29cdbf`). **C-2 built:** Questionnaire tab = V2 shell (11 topics, drafts, explicit Save → one Life Record PATCH); Earlier answers + Operator Intake read-only. 25 DOM checks against the real writer, 12 mutations caught |
> | **authority inventory** | 2026-09-24: every biography surface/writer named with a C or D destination + the C (operator) and D (runtime) authority gates — checkpoint "Legacy-surface and authority inventory" |
> | **C-2 status** | **ACCEPTED / CLOSED 2026-09-24**: R7 live PASS (PATCH 200, rev 1→2, reload persistence, no QF takeover, zero view writes) · `.venv` 119 OK, 0 skips, 1 expected failure · Operator Intake 5/5 | (checkpoint "C-2 — LIVE ACCEPTANCE FAILED", then its repair and acceptance) |
> | **C-2b** | **ACCEPTED / CLOSED 2026-09-24** (`be6b8b1`). `.venv` 45 OK, 0 skips; JS preservation gates green (graph 52/52, Operator Intake 5/5, persist-empty 17); live Chrome no-write sweep. Bio Builder = Questionnaire · Sources & Notes · Review · Family (DRAFT) · ⋯ Legacy (READ ONLY); header `<name> · Life Record revision N`; Life Threads / Reset Identity / Intake tab out of navigation; earlier-system writes (Suggestion Accept, Source-claim Correct, Conflict Replace/Merge, WO-13 Promote) **HELD** by `life-record-authority.js`, fail closed, held items stay pending; Family persistence local-only; narrator switch creates no `lorevox_qq_draft_`; `questionnaire_first` retired and unselectable. Gate: `tests.test_bb_consolidation` (37 checks, 12 mutations caught, 1 equivalent recorded). Record: checkpoint "C-2b" |
> | **C-3** | **BUILT 2026-09-24 — `.venv` 214 OK, 0 skips, 1 expected failure; live Chrome PASS (one PATCH per Save, same ids after switch and reload, Family derived, correction with `expectedPrevious`, rich intake refused 422). **ACCEPTED / CLOSED 2026-09-24** (with the post-commit follow-up below).** Questionnaire V2 edits people and relationships (stable ids, whole names incl. former/variant, preferred name, pronouns, roles with no preselection, qualifiers, lineage side, what the narrator calls them, period as said, life status, stated counts) — one PATCH per Save through the writer. **Family is DERIVED FROM THE LIFE RECORD** (read-only, "Edit in Questionnaire"); the old Family editor's handlers are unexported and earlier `lorevox_ft_draft_*` drafts are kept, never promoted, never shown. New model rule: no relationship stored twice. **Real-narrator creation no longer seeds biography**: `POST /api/people/intake` refuses family/marriage/children/work/service/faith/today sections unless `testing_only` (the Lori harnesses, until D). Every id — people, names, relationships, assertions — is minted before Save, so a retried Save is refused whole, never duplicated. Supervisor review found 3 defects (stated 0 passed the intake guard; Family A→B→A stale response; duplicate rule refused an unknown-dates remarriage) — all repaired and pinned. Gates: `tests.test_qv2_people` (37 DOM checks + database checks), 21 mutations caught; `tests.test_bb_consolidation` 41; `tests.test_intake_no_biography_seed` 5 (route tests — `.venv`). Record: checkpoint "C-3" |
> | **C-3 follow-up** | **ACCEPTED / CLOSED 2026-09-24.** C-3 committed and pushed (`e387522`, `8a87738`); the post-commit supervisor review found two transition defects — read-side `child_of`, and pre-C-3 drafts without pre-minted assertion ids. One narrow corrective commit on top; no history rewritten; C-3 not reopened as a phase. **Final gate: focused `.venv` 18 OK, 0 skips (the 5 intake route tests ran); `test_qv2_people` 45 checks; 26/26 C-3 mutations caught.** The earlier full `.venv` (214 OK, 0 skips, 1 expected failure) and the live Chrome acceptance stand and were not rerun. Record: checkpoint "Post-commit follow-up" |
> | **C-4** | **ACTIVE.** Date contract DECIDED and recorded (C-4A): `{text, value, precision}` — text as entered, value from a bounded EDTF profile or null, precision `day/month/year/unknown`; approximation/uncertainty only in the value; server validates text/value/precision agreement; legacy dates read and never rewritten. Shared corpus `tests/fixtures/life_record_dates_v1.json` (C-4A `a787f64`). **C-4B ACCEPTED 2026-09-24** — built, then repaired after the pre-commit patch review (2 defects: a refused DOB still stored in the legacy `people` row; the editor re-parsing an unchanged legacy period end). One parser per runtime, both reproducing the corpus; the writer checks every added assertion whose catalog `value_type` is date/date_interval, and only the relationship/name period endpoints a write changes; both creation routes keep a refused or range DOB out of the legacy row (and the intake profile mirror) and report `notRecorded`; the editor carries an unchanged legacy period end as stored; `age_at` reads the value. **`.venv`: 125 tests OK, 0 skips** (the two bridge-sync tracebacks are deliberate injected failures in `test_c2_bridge_order_swap`; suite OK); date mutations **27/27**; targeted people mutations **4/4** (incl. `legacy-period-reparsed-on-unrelated-edit`); design validator 20 · 14 · 22 · 11 **DESIGN COHERENT**; design mutation gate every mutation caught; final pre-commit patch 20 diffs, `git diff --check` clean. Committed and pushed: `ac2aad0` (code), `b347854` (docs). Record: checkpoint "C-4B" |
> | **C-4C** | **ACCEPTED / CLOSED 2026-09-25.** Questionnaire editability now comes from what V2 can write today (one strict-JSON capability declaration read by both the editor and the catalog compiler); catalog editable 61 → 12; `event.work.period`, `event.education.period`, `event.separation.date` added; event-date maps gain separation, education, work and activity. `.venv`: 150 tests OK, 0 skips; catalog current; design 20 · 14 · 22 · 11 DESIGN COHERENT; catalog mutations 28/28; capability mutations 16/16, no equivalents; `git diff --check` clean; 15-diff patch — product and documentation reviewed and accepted. Record: checkpoint "C-4C" |
> | **C-4D** | **ACCEPTED / CLOSED 2026-09-25.** `.venv`: C-4D same-process bank 179 OK (runner, 0065, historical migrations, Life Record, capabilities, trips, hygiene, isolation); trips ⇄ hygiene 16 OK both orders (FAILED before the repair); package/erasure 110 OK; catalog current; DESIGN COHERENT; mutations 19/19 (R1–R8, M1–M8, H1–H3); anchors intact. Runner repaired (one transaction per runner-managed file with its tracking row; `-- migration: foreign_keys=off` protocol refusing new FK violations; foreign keys always restored) after 6/11 scratch tests proved the gap; `0065_life_record_activity_event.sql` admits `activity`; a confirmed test-isolation defect (`test_db_connection_hygiene` replacing `api.db`) repaired test-only so trips preservation holds in one interpreter; 19 mutations. Record: checkpoint "C-4D". Scope as issued — migration-runner atomicity/FK precondition (pin its behaviour on scratch databases, narrowest repair), then the `activity` event type at the next free migration number, re-derived just before it is written; scratch databases only, stack down |
> | **C-4E** | **ACCEPTED / CLOSED 2026-09-25** (repaired after pre-commit review; MAG-Chris rerun complete; ChatGPT ACCEPT). Review found one defect — correcting an explicitly ACCEPTED vital assertion left the acceptance on the superseded one (accepted DOB lost its anchor; accepted "living" + death refused the Save); repaired in the shared `answerOps` by moving the acceptance in the same Save, pinned by a regression and mutation V11 (vital gate now 11). First MAG-Chris run before the repair: 192 OK, 10/10, 16/16. **MAG-Chris after the repair: 194 OK, 0 skips; catalog current; DESIGN COHERENT (21 rules); 11/11; 16/16; anchors intact.** Birth and death editable in V2 on canonical events (date under the C-4 contract; place chosen by id or created new, never merged by name; existing event extended, never duplicated — new writer rule "one birth and one death per person"; a death records `deceased` in the same Save; deceased alone invents nothing). Catalog editable 12 → 16. 11 mutations. Record: checkpoint "C-4E" |
> | **C-4F** | **BUILT 2026-09-25 — MAG-Chris `.venv`: 225 OK, 0 skips; catalog current; DESIGN COHERENT; gates 20/20 · 11/11 · 16/16. Review: the pre-existing 0065 test failure blocked acceptance → TEST repaired (each old type on a fresh person; rule and 0065 unchanged) — `.venv` 6/6. **Pre-commit review repair 2026-09-26:** participant correction on the same event; new writer rule — derived relationships match their event's people (updated atomically); Separation form offers no Place; "to present" ⇄ Current refused when this edit creates the contradiction; `event.separation.participant`; kind of home on a new home. **MAG-Chris `.venv` after the repair: 0065 6 OK; 232 OK, 0 skips; catalog current; DESIGN COHERENT (22 · 14 · 23 · 11); gates 32/32 · 11/11 · 16/16; diff-check clean.** Awaiting ChatGPT final review. NOT committed.** Homes, unions and separations editable in V2 as their own occurrences (place by id or new, never merged; periods / dates under the C-4 contract; the same pair never deduplicated; a separation never rewrites its union; an edited occurrence keeps its id). **Current home is ONE explicit truth** (`attributes.current`), never read from a missing or open end. Kind of home; **stated marriage count** (`person.reported_count.marriages`, new concept). **Intake `current_residence` now established in the Life Record** (one place, one current home, no invented date). Person-independence scenario (transition, former name, biological children, current wife) pinned. Catalog editable 16 → 24. 20 mutations, 20/20 in the sandbox. Reported, not invented: place `parts` contract, childhood flag. Found, not caused, test repaired: a C-4D test red since C-4E. Record: checkpoint "C-4F" |
> | **next** | **ChatGPT review of C-4F**; then C-4G (education as repeatable occurrences — hard requirements recorded in checkpoint "C-4F") | **D carries the WHOLE authority ledger, checkpoint B1–B17**, not a short list. The real-narrator identity floor stays an **open D-0 decision** |
>
...
>
> **TWO-MACHINE AUDIT — CLOSED 2026-09-10.** Eleven labelled Phase 0 reports plus `scripts/phase0_report_comparator.py` run `phase0-report-comparison-20260910T234004Z`, all under `.runtime/eval/` (never staged). Result in WO §31: the family ids are identical on both machines; each narrator has lanes on only one of them (Chris: conversations / bio facts / audio on the desktop, the entire travel domain on the laptop). **Neither machine is "the copy"; Phase 6 exports from both into separate clean roots.** *(This row said "Melanie is laptop-only" until 2026-09-12. **Corrected by Chris:** treat her as present on BOTH origins until the two-origin comparison proves otherwise. The laptop's `people` table does carry her — `d56900b5` — but no desktop read in this session tested the claim, and an unproven absence is not a finding. The comparison settles it.)* Laptop handoff `docs/handoffs/HANDOFF_2026-09-10_LAPTOP-NARRATOR-AUDIT.md` is done and closed. No further audit questions unless a generic package invariant needs one.
>
...
>
> **PHASE 5a LANDED 2026-09-11 — WO §35, §35.1, §35.2.** The Narrator Data Center entry on the Operator tab (`#lvOperatorPortableNarrator`, `HORNELORE_OPERATOR_PORTABLE_NARRATOR=1`): router `/api/operator/narrator-package` over the one service; export as a durable `0056` job with stage progress and a download streamed from outside `DATA_DIR`; import streamed to staging outside `DATA_DIR`, answered with integrity and readiness as separate verdicts and a server-computed stepper; restore only while READY behind a native confirmation dialog; remove-server-copy with the record kept; Activity from the two ledgers. API suite green under `.venv` (see WO §35 for the count on acceptance). **Phase 5b (rich per-domain inspection) is post-Phase-7 and never blocks Phase 6 or the cutover.**
>
> **PHASE 5a LIVE ACCEPTANCE WALKED 2026-09-11 IN CHROME — ACCEPTED, WO §35.3.** All fourteen points pass on synthetic Ada `8ec7a427`: package on the live root, download, remove-server-copy, then a clean root (`/mnt/c/hornelore_clean_5a`) with two verdicts, the seven-step stepper, the dialog with Cancel focused, restore with ids verbatim, and the second import refused as a collision with no Restore control (a forced POST is 409). **Four repairs came out of it, all product:** the card ignored the picker's narrator (switch hook fired before `state.person_id` was set); the Move view lost a durable job on reload; **dry-run matched installation dependencies on the primary key instead of the referenced column — every clean installation refused every narrator with a questionnaire**; a clean root had no `chat_ws` plan until someone chatted. The last two are the Phase 7 cutover case and would have been met with real narrators. Pinned by two production-boundary tests in `tests/test_narrator_package_restore.py`. `bagit` had to be installed into `.venv-gpu` (pinned, never installed).
>
> **Cleanup decided 2026-09-11:** the seven Ada rows are Block D fixtures of `WO-LORI-ARCHIVE-TO-MEMOIR-02` (held by its handoff), not 5a data — none erased; `8ec7a427`'s changed state recorded in that handoff §7. Phase 5a removed only its own clean root, package copies and staging. **Transition block DONE 2026-09-11:** `requests==2.33.1` (chardet 7 via readability-lxml tripped 2.32.5's bound), `TRANSFORMERS_CACHE` dropped from `.env.example` and the shell profile, `launchers/hornelore_run_clean_root.sh` (refuses the real `DATA_DIR`, a populated root without `--reuse`, a busy port; own `<root>.packages` / `<root>.staging`); one stack start with neither warning. **PHASE 6 DESKTOP ORIGIN — ALL THREE PASS, 2026-09-11 (WO §36.1–36.3).** Christopher `ea4ae5d6afc5` (86 rows / 112 files / 31.5 MB), Kent `451876ad9efa` (56 / 62), Janice `74252b2e79c5` (5 / 51): each restored into its own clean root via the launcher, re-exported with **no narrator interaction**, and compared `EQUIVALENT` with only the schema fingerprint informational. One more product defect found and fixed on the way (E: a never-initialised root refused `db_not_found`; the router now runs `init_db()` before any verdict), one comparator correction (empty lane = absent lane), and one finding proven **not** to be portability: opening a narrator appends four `bio_facts` through `questionnaire_put` (BACKLOG §5). Baseline for the two-origin comparison: `.runtime/phase6/desktop_christopher_baseline.json`. Clean roots `hornelore_clean_{christopher,christopher_probe,kent,janice}` left in place until the two-origin comparison. **PHASE 6 LAPTOP ORIGIN — ALL THREE PASS, 2026-09-12 (WO §36.5).** Christopher `a2f360689b58` (**533** rows / 214 files / 123.5 MB), Kent `f42a80afb420` (43 / 45), Janice `aca0cdfd9c13` (66 / 21): each restored into its own clean root via the launcher, re-exported with **no narrator interaction**, and compared `EQUIVALENT` with only the schema fingerprint informational. **Both origins are now proven independently.** The laptop found one product defect the desktop data could not reach — **ownership closure**: 20 Christopher `trip_turn_links` named 9 residue `sessions` (`person_id` NULL, pre-0044) holding 100 turns, and the §30 guard refused 60 unresolved references. Correctly. Fixed by a new `DirectOrExclusiveInbound` owner type on the `sessions` lane — a residue row reached EXCLUSIVELY from one narrator's owned rows is that narrator's **by derivation, never by writing `person_id`**; ambiguous rows stay outside the closure and the existing guard refuses rather than choosing an owner. Export/erasure parity was NOT free — hard deletion does not consume the declaration (§17), so `db.py` now does for derived-closure lanes only, resolving the closure BEFORE any deletion (deleting `trips` first cascades away the `trip_turn_links` that prove reachability). Reach **9 of 911 residue sessions = 0.99%**; blast radius measured across all 74 people — **73 zero delta, 0 unexpected, none losing rows**. `.venv-gpu`: residue suite 18/18, five-suite bank 90/90, 0 skips. **109 narrator-owned rows that were invisible to both portability and erasure are now under the same rule.** The **three laptop** source packages are preserved in the laptop's `/mnt/c/lorevox_packages` (verified on disk 2026-09-12); the **three desktop** packages are preserved in the DESKTOP's own `/mnt/c/lorevox_packages` and are **not on the laptop**. Six exist, on two machines — the two-origin comparison needs both sets in one place, which is a transfer step nobody has run yet. **TWO-ORIGIN COMPARISON TOOLING BUILT 2026-09-12** — `scripts/two_origin_compare.py` (read-only, packages only, no database, no restore, no merge) + `tests/test_two_origin_compare.py`. Three keys are derived **separately** per table: physical, ownership path (reference only), and cross-origin logical key — which is frequently **absent**, and absence is a verdict. Two rules, both corrections to a first draft that would have produced a misleading merge design: **content is never part of a logical key** (a changed row would change its own key and hide the conflict), and **a UUID is not automatically a cross-origin identity** (`people.id` earns one because §31 *measured* the same id on both machines; `photos.id` does not). `turns` = `NO_SAFE_CROSS_ORIGIN_KEY` — no stable per-conversation ordinal (`db.py:587`) — so turns merge by conversation, never paired. **The finding that shapes Merge/Remap: `turns.id` is stored in SEVEN places and a `PRAGMA foreign_key_list` walk finds TWO** (two undeclared `story_candidates` INTEGER columns from 0047, two `turnrow:<id>` TEXT keys, and a `bio_facts.source` JSON field). **A merged database can therefore pass `foreign_key_check` and still be broken** — Merge/Remap owes a semantic reference validator. The comparator also audits, per package, whether every turn reference resolves inside it, which tests the exporter's own §30 guarantee across the *full* set rather than the declared subset. Draft WO: `docs/wo/WO-LOREVOX-MULTI-ORIGIN-MERGE-REMAP-01_Spec.md` (design only, nothing implemented). **Melanie is OUT OF SCOPE** by Chris's decision 2026-09-12 — desktop `3fc781ae`, laptop `d56900b5`, distinct records, logical-person equivalence deliberately unresolved because she is to be removed rather than migrated; no deletion in this lane. **TWO-ORIGIN COMPARISON COMPLETE AS AN EVIDENCE PHASE, 2026-09-12** — all three pairs compared read-only, all six packages byte-identical before and after, reports in `.runtime/two_origin/reports/`. Christopher: 10 desktop-only / 9 laptop-only keyed rows, 4 same-key-different-content, **31** `NO_SAFE_CROSS_ORIGIN_KEY` tables, **remap required**, 110/212 one-side-only files. Kent: 4 conflicts, 10 no-safe-key tables, no surrogate collision, 60/43 files. Janice: 1 laptop-only row, 4 conflicts, 13 no-safe-key tables, no collision, 49/19 files. Each pair has exactly 2 same-path/different-byte files, both derived archive files — **`index.json` is fully regenerable** (`_register_session_in_index` writes only fields already in `meta.json`, `archive.py:1217`/`90`) and is NOT a narrator-content conflict; **`rolling_summary.json` is NOT regenerable** (LLM-produced, pruned, contamination-filtered — `archive.py:654-687`) and is a real divergence requiring policy.
>
> **⚠ PORTABLE NARRATOR INTEGRITY OBLIGATION REOPENED (Christopher, laptop origin only).** The full-reference audit found **9 dangling** `turn_extraction_ledger` `turnrow:<turns.id>` references in the laptop's Christopher package — accepted by the old exporter because encoded TEXT references sat outside its §30 closure model. **Precise status:** the desktop Phase 6 proof for all three narrators stands; the laptop Phase 6 proof for **Kent and Janice** stands under the expanded audit; **laptop Christopher's** export→restore→export equivalence remains historically true *under the previous validator*, and the package is **structurally valid but semantically incomplete under the newly discovered invariant** — not a corrupted zip. It **must be investigated and re-exported before it is a Merge/Remap input.** **Repair landed 2026-09-12:** `narrator_data_inventory.ENCODED_REFERENCES` declares the TEXT/JSON forms and two new `ColumnRef`s cover `story_candidates`' 0047 turn columns; the exporter consumes the declaration, and a malformed encoding refuses rather than being skipped. **The repair itself shipped a defect that only the POSITIVE tests caught, and it is the more useful half of the lesson:** `parse()` returned the id as a *string* while `turns.id` comes back from SQLite as an *int*, so `"1455" in {1455, …}` was always false and the checker refused **every legitimately resolvable encoded reference** — i.e. every narrator who has ever had a turn extracted. Three tests failed, all of them "a valid reference passes"; every refusal test stayed green, because a refusal test cannot tell you *why* it refused. The first hypothesis on review was an unclosed fixture, and closing the fixture would have turned the suite green over a checker that blocked real exports. Fixed by one production line (`int(rest)`), fixture untouched — and that experiment is the proof. Encoded-reference suite **20/20**. **What the packages cannot prove** — whether those turns ever existed on the laptop, whether they are Christopher's, or whether the ledger rows are stale — needed a read-only investigation against the laptop's live database. ***That investigation is DONE and committed 2026-09-13*** (`09044ed` · `b64bc85`); findings in `docs/wo/WO-LOREVOX-PORTABLE-NARRATOR-01_SESSION-OWNERSHIP-FINDINGS-2026-09-13.md`, dry-run repair in `..._CHRISTOPHER-REPAIR-PLAN-2026-09-13.md`. **A reference outside the owned closure is a REFUSAL, never a reason to widen ownership** — and it stayed one: nothing was inferred from prose or from archive-directory proximity, which 0044 explicitly refuses.

**WHAT THE INVESTIGATION FOUND.** The nine references were the symptom; **three whole NULL-owner `sessions` (28 turns)** were the omission. Ownership is settled **structurally** — each has exactly one `interview_sessions` row naming Christopher, 0044's strongest recorded link. The migration **runner is exonerated** (`executescript` runs the whole file; failure re-raises leaving no tracking row), so **`applied_at` proves only that nothing raised, never that an UPDATE matched a row** — 0044's backfill yielded at most **1** row of its 81 candidates. The evidence it needed *existed* when it ran, proven from code not timestamps (`ensure_interview_session` is `INSERT OR IGNORE`, so it can never fill a NULL owner later; `started_at` is generated at that insert). **The exact failure mechanism is NOT recovered and is not claimed.** **Del is NOT an active writer regression** — fix in tree 2026-08-16 13:49 UTC, Del's session 08-17 00:41, first row proving the fixed writer actually ran 08-18 01:36: **pre-deployment-indeterminate**. Of 912 ownerless sessions, **81** have a unique accepted owner and they are **not 81 lost conversations**: 36 belong to synthetic personas (Walt replayed seven times at *exactly* 14,420 chars is a harness fingerprint), and most of Christopher's 42 are the archaeology of building the travel-document, photo, guard and safety systems with himself as narrator. **Janice's omitted session has ZERO narrator-authored speech** (its only user row is an injected `[SYSTEM:]` return-context message) and **Kent's is a harness fixture duplicated elsewhere — neither package is rebuilt on account of this.** Net salvage: **28 "missing" turns and nine dangling references reduce to one genuine family fact plus one photo correction.** **Withdrawn in writing so they are not re-cited:** a circular rowid watermark, a false-negative `reached by: []` (the ledger's column is `session_id`, not `conv_id`), and two classifiers that let autobiographical vocabulary outrank structural test provenance.
>
...
>
> | narrator | authoritative package | restored | re-exported |
> |---|---|---|---|
> | Christopher `a4b2f07a` | `Christopher_Todd_Horne_24560db21dd6.lorevox.zip` | 524 records / 214 files | **EQUIVALENT** `tables=60 rows=524 files=214` |
> | Kent `4aa0cc2b` | `Kent_f42a80afb420.lorevox.zip` | 43 / 45 | **EQUIVALENT** `tables=60 rows=43 files=45` |
> | Janice `93479171` | `Janice_aca0cdfd9c13.lorevox.zip` | 66 / 21 | **EQUIVALENT** `tables=60 rows=66 files=21` |
>
> **ACCEPTED 2026-09-14 against a fresh product-initialised root** (`/mnt/c/lorevox_family`, `init_db()` only — Merge/Remap never initialises a root, §5a). All three dry-ran `RESTORE READY` and restored through the ordinary `scripts/narrator_package.py` path, **sequentially into the SAME installation**, Kent arriving into a root that already held Christopher and Janice into one holding both. `scripts/family_root_verify.py` then read that root **read-only** (`mode=ro`) and measured: **3 narrators; 524 / 43 / 66 rows counted through `narrator_data_inventory`'s own `owner_predicate`; 0 orphaned owners; 0 `foreign_key_check` violations; 0 dangling semantic references; 3 `complete` restore jobs; 0 merge jobs; 280 payload files** (214 + 45 + 21, reconciling exactly). Each narrator was then independently re-exported and compared `EQUIVALENT`, with only `source_commit`, `source_schema_migrations` and `source_schema_fingerprint` informational — the packages came from older commits and the root is at `3b187cb`. **The round trip is also the proof that later restores did not disturb earlier narrators**: Christopher compares equivalent to his own package after two more narrators landed on top of him.
>
> **WHY MERGE/REMAP IS NOT NEEDED HERE, stated so nobody re-derives it:** all three packages were exported from ONE laptop database, so `turns.id`, `turn_extraction_ledger.id` and `turn_extraction_results.id` came from single AUTOINCREMENT sequences and **cannot collide with each other**. That is exactly the case WO §34 carved out — *same-origin packages coexist; multi-origin may collide and V1 refuses*. Sequential restore is the supported path, not a workaround.
>
> **MERGE/REMAP IS BUILT, HARDENED AND OFF THE CRITICAL PATH.** It remains a supported capability for genuinely combining two independently developed histories of one narrator, or importing into an installation with colliding installation-local ids. It is **no longer a prerequisite for these three narrators or for Phase 7**, and the desktop-vs-laptop comparisons and the Christopher adjudication packet are **historical engineering evidence only** — do not spend further time reconciling those pairs. *(This block previously sequenced "Merge/Remap implementation → Phase 7" and the checklist called it required before the one-root cutover. Both were true when written and are now false.)*
>
...
> 4. **Only now erase.** `DELETE /api/people/{id}?mode=hard` runs the saved erasure plan (`POST /api/people/{id}/erase-retry` if it reports partial). **Soft delete is not erasure** — it hides the narrator and leaves every byte on disk. *(This said `/retry-erasure` until 2026-09-15. **That route does not exist and never did** — the shipped router is `routers/people.py:317` `@router.post("/{person_id}/erase-retry")`, and `db.py:6483` returns the same string in its own partial-erasure payload. Corrected before the laptop session; the wrong form would have 404'd at the one moment a partial erasure needed completing.)*
...
> ---
>
> **SUPERSEDED — the Merge/Remap implementation record, kept because it is real and hardened (2026-09-14).** *(This block read "design may continue; **implementation waits** on the Christopher disposition" with a five-step list whose first three steps were already done by the time it was read. The disposition is closed, the replacement comparison has run, and nothing is waiting — the exact stale-instruction defect `CLAUDE.md` opens by warning about.)* **Steps 1–3 are DONE:** the desktop tree reconciled at `origin/main`; `Christopher_Todd_Horne_24560db21dd6.lorevox.zip` brought across (`C:\lorevox_two_origin`, staged into `.runtime/two_origin/laptop/`, the superseded `a2f360689b58` retained as evidence); **Christopher's comparison re-run 2026-09-14** with every substantive number identical to the pre-repair baseline — 10 desktop-only / 9 laptop-only keyed rows, 4 same-key conflicts, 31 `NO_SAFE_CROSS_ORIGIN_KEY` tables, one physical collision (`turn_extraction_results.id = 1`), 110/212 one-side-only files, 2 same-path/different-byte archive files, **zero dangling references both origins**, `Remap required: YES`, inputs byte-identical after the run. **Kent and Janice stand and are NOT re-run.** **An evidence-tool drift was corrected on the way:** `two_origin_compare.py` hard-coded five reference sites as undeclared after the 2026-09-12 repair declared them, with a regression test pinning the stale answer and the WO spec repeating it — one defect in three places, fixed together at `37b33ca`; `declared` is now derived from the live declaration, a parity test holds the seven independently discovered sites against the seven production-declared ones, and the FK-walk count is corrected from "two" to **none** (no migration contains `REFERENCES turns`). **The §3d closure hunt the spec owed is DISCHARGED:** three narrator-owned integer surrogates exist, not one — `turns.id` (seven references), `turn_extraction_ledger.id` (one, a real FK), `turn_extraction_results.id` (**zero**, renumber only) — and **no filesystem path encodes an integer row id**, so the merge rewrites columns and renames no files. One latent defect registered, not fixed here: `media.filename` is an absolute path in a portable lane with no `path_columns` declaration (`routers/media.py:108` vs `narrator_data_inventory.py:250`); no `media` rows exist in any of the six packages. **NEXT:** Merge/Remap implementation in the order of spec §6 — synthetic two-origin fixtures with deliberate collisions, deterministic remap tables, full semantic-reference rewrite, semantic validator, dry-run that writes nothing, refusal on every conflict class, rollback/recovery and target-root isolation — **then** a Christopher rehearsal from the two real packages into a brand-new disposable root, **then** Phase 7. **The repair closeout and the merge implementation are separate work and are not mixed.** *(This list previously sequenced an ownership repair, a curation decision on four candidate sessions, and salvage items — **all superseded**. The portability fault was nine stale derived ledger rows and nothing else; the curation analysis is Phase 7 input and gates nothing. It also asserted "a session left NULL stays out of the package by construction", which is **false** under the shipped declaration: `DirectOrExclusiveInbound` admits a NULL-owner session that a Christopher-owned `trip_turn_link` reaches exclusively, and the 533 baseline already contained 9 such sessions and 100 turns. The 578/597/654 figures rested on that error and are withdrawn; the measured result is **524**.)* Superseded: "run the three real comparisons" — done; its output is the input to the Multi-Origin Merge/Remap WO, which is designed after it and not before, then implemented and proven on SYNTHETIC two-origin data with deliberate collisions, then rehearsed into a brand-new root from packages only. **Only then Phase 7** — the clean Lorevox data world: immutable backup of `/mnt/c/hornelore_data` left untouched, a new root initialised by the product, `DATA_DIR`/`DB_NAME`/the other path variables reconciled so no mixed root is accepted, and a mutually collision-free package set restored into it (§17, §27.6–27.7). **Phase 7 cannot precede the merge for Christopher** — his conversations and audio are on the desktop and his entire travel domain on the laptop, so a one-root family installation needs both halves of him. *(This sentence read "Then Phase 7 — real-family packages, each into its own clean root, both machines" until 2026-09-12: that is Phase 6's description, and Phase 6 is done. Corrected on review.)* **Lori conversational Phase 6 (Block D) STAYS PAUSED for portability** — verdict recorded, `docs/handoffs/HANDOFF_2026-09-09_PHASE6_C-BLOCK-AND-LEAN-QUALITY.md`.
>
> **NARRATOR UI — `WO-LORI-NARRATOR-FOCUS-MODE-01` IMPLEMENTED 2026-09-12, STRUCTURAL 36/36, LIVE ACCEPTANCE PARTIAL — NOT ACCEPTED** (checklist row 0b; ledger in the spec's status block). Live-proven: cases 1, 3, 4, 5, 6, 9, 10, 11 on a disposable ZZ narrator. **Owed:** case 2 retest after the FocusCanvas tab-order repair, case 7's spoken turn (needs Chris), case 8 at exactly 697px and 650px. **UI work is deliberately stopped here** — the current lane is the two-origin comparison below. Original spec docs-only at `3a62b38`. Drafted and revised 2026-09-12 against the shipped page: a presentation-only mode of the existing narrator room — large print, one conversation, nothing else on screen, entered by the operator and left by a deliberate gesture the narrator cannot produce by accident. No server code, no prompt change, no extractor change, no second turn path. **It is NOT Phase 5b** (the Rich Data Center, post-Phase-7) and must not be conflated with it: when Chris says "the UI work", this is the WO he means. It is independent of the portability lane and waits on his go, not on Phase 7.
>
