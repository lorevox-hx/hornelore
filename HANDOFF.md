...
> | **Guard Lab — what is NOT built yet** | The Operator API and Guard Lab panel; atomic `All Switchable Off` / `Restore Defaults` controls; optimistic-revision UI; and the LIVE acceptance that needs a running stack — no-restart toggling, the mid-turn freeze observed rather than only structurally proven, the stale-client clamp and restart persistence. **The `chat_ws` behavioural suites cannot run in the agent sandbox** (`transformers` absent, identical at HEAD), so `.venv-gpu` in WSL is their only verification. Then the lean baseline prompt, a populated synthetic Walt, and the owed §5 short-natural run. **No Walt/John run until `All Switchable Off` is truthful across all four surfaces** |
