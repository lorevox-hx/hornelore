...
> # ▶ CURRENT ACTION — THE LORI GUARD LAB (`WO-LORI-BASELINE-RESET-AND-GUARD-LAB-01`). THE WALT+JOHN MEASUREMENT IS CLOSED. ARCHIVE-TO-MEMOIR 0–4 AND 5A/5B ARE ACCEPTED; 5C IS QUEUED.
>
> | | |
> |---|---|
> | **Current action** | **BUILD THE GUARD LAB CONTROL PLANE. Do not rerun the Walt+John measurement; do not start 5C; do not run a live Walt baseline yet.** The diagnostic's own order of work is being executed and is PARTLY DONE — see the row below. The detour is deliberate: the evidence said the dominant cause of what narrators received was the post-generation control layer, and there was no way to test 43 accumulated interventions one at a time. The Guard Lab is that instrument. Spec: `docs/wo/WO-LORI-BASELINE-RESET-AND-GUARD-LAB-01_Spec.md` |
> | **The diagnostic's four items — where they actually stand** | (1) **John's `META_FEEDBACK/correction` misroute — DONE**, pushed `8fa2e4e`. The last un-narrowed `not X but Y` pattern; two independent discriminators; `api.log` shows the misroute happened **three** times, including once on 2026-08-31 in an unrelated session. (2) **Response-trace closer — STILL OWED, untouched.** `close()` remains reachable only from `turn_extraction.py`. (3) **The control layer — IN PROGRESS**, as the Guard Lab. (4) **Prompt budget — unchanged**, still second-order |
> | **Guard Lab — what is BUILT and pushed** | **43 numbered intervention authorities**, derived from the tree and then numbered: 10 PROMPT, 7 ROUTE, 11 TRANSFORM, 6 VALIDATE, 3 REPLACE, 1 FINAL_WRITER, 5 LOCKED. Population **37 SWITCHABLE / 6 PROTECTED / 0 PENDING_SEAM**, derived — never written into runtime logic. Three-state resolution (canonical default / operator override / effective state + reason), two fingerprints plus a revision, migration `0053` with durable operator overrides, and turn-authority acquisition wired into `chat_ws` with the deterministic route family clamped. `scripts/lori_guard_catalog.py` generates the human-readable catalog from registry metadata — there is no second hand-maintained list |
...
> | **Guard Lab — what is NOT built yet** | Prompt authorities (1, 3–11) do not yet read the snapshot; `comm_control` ids 32–42 are registered and separable but not yet selected per turn; post-generation and final-writer consumers are unwired. **The id 54 seam must wrap `finalize_presentation()` ONLY** — the existing `delivers_question()` containment at `chat_ws.py:6933` is what stops the protected ledger stamping a presentation the narrator never received. Operator API, Operator panel, counterfactual attribution, atomic batch and the live no-restart proof all remain. **No Walt/John run until `All Switchable Off` is truthful across all four surfaces** |
...
> | **Two instrument findings** | (a) **The response trace has no closer on the harness path.** `close()` is reachable only from `turn_extraction.py`, which this client never invokes; records survive only via the 180-second parked sweep, which a 60-second run never trips for its own turns. Walt's evidence existed only because John ran afterwards; John 2's did not survive. (b) **The deterministic witness path produces no trace at all** — it returns before the generation section where the trace opens. Defensible, but a report must name which turns are absent and why |
