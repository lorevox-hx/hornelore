...
> | | |
> |---|---|
> | **Current action** | **ANALYSIS AND FIXES FROM THE WALT+JOHN EVIDENCE. The measurement is DONE — do not run it again.** Full report: `docs/reports/WALT-JOHN-VRAM-PROMPT-BUDGET-DIAGNOSTIC.md` (local-only). Order of work, from what the evidence says: (1) **John's opening chapter was classified `META_FEEDBACK / correction`** — 238 words of autobiography answered with a title-cased fragment of his own sentence, identically on both runs, sub-second, no generation. A routing defect before generation and the worst thing that happened to a narrator in the dataset. (2) **Give the response trace a closer** when extraction is skipped — see below. (3) **Then the control layer**, with numbers in hand. (4) The prompt budget is real but second-order to (3) |
