...
>
> **NOW — ON THE DESKTOP (moved 2026-09-14; the laptop lane is closed, and we return to it only for a source-DB write or export).** Merge/Remap **design may continue**; **implementation waits** on the Christopher disposition. **NEXT ACTIONS, in order:** (1) reconcile the desktop tree — `git fetch` and compare local against `origin/main` **before** any pull, because the desktop previously held the semantic-reference edits uncommitted and those may now be the same work already on remote; (2) **decisions only Chris can make**, from the repair plan — the four §1b candidate sessions, salvage items [1218] and [1448], and the §4 choice on the nine ledger references (**the plan argues C+B**: declare `turn_extraction_ledger → turns` in `COLUMN_ONLY_REFERENCES` so the guard covers that edge whatever else happens, then remove the stale derived rows — **rather than importing 28 development turns into canonical history**); (3) **back on the laptop** for the narrow ownership repair and the new authoritative Christopher package — **the desktop must not pretend it owns `/mnt/c/hornelore_data`**; (4) bring only that replacement package across, rerun Christopher's clean-root restore / no-interaction re-export / semantic compare / full reference audit; (5) Christopher's two-origin comparison **only**; (6) only then Merge/Remap implementation. **Ownership repair and preservation are the same list** — a session left NULL stays out of the package by construction, so **no deletion is needed to keep the package clean**, and **35 of Christopher's 42 are simply left alone**. **Salvage happens through the product, never by SQL** — a hand-written trip note is exactly the unprovenanced data this work exists to remove. Pre/post record counts are recorded in the plan so the repair is checked against a **number**, not an impression (533 today; 578 / 597 / 654 by scenario; **files unchanged at 214** either way). Superseded: "run the three real comparisons" — done; its output is the input to the Multi-Origin Merge/Remap WO, which is designed after it and not before, then implemented and proven on SYNTHETIC two-origin data with deliberate collisions, then rehearsed into a brand-new root from packages only. **Only then Phase 7** — the clean Lorevox data world: immutable backup of `/mnt/c/hornelore_data` left untouched, a new root initialised by the product, `DATA_DIR`/`DB_NAME`/the other path variables reconciled so no mixed root is accepted, and a mutually collision-free package set restored into it (§17, §27.6–27.7). **Phase 7 cannot precede the merge for Christopher** — his conversations and audio are on the desktop and his entire travel domain on the laptop, so a one-root family installation needs both halves of him. *(This sentence read "Then Phase 7 — real-family packages, each into its own clean root, both machines" until 2026-09-12: that is Phase 6's description, and Phase 6 is done. Corrected on review.)* **Lori conversational Phase 6 (Block D) STAYS PAUSED for portability** — verdict recorded, `docs/handoffs/HANDOFF_2026-09-09_PHASE6_C-BLOCK-AND-LEAN-QUALITY.md`.
>
