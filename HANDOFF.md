...
> | **next** | **C-4C — BUILT / TESTED 2026-09-25. Product code has passed supervisor review; documentation closeout pending final pre-commit verification (not yet ACCEPTED / CLOSED).** `.venv` (Chris, laptop): 150 tests OK, 0 skips; catalog current; design validator 20 · 14 · 22 · 11 DESIGN COHERENT; catalog mutation gate 28/28; QV2 capability mutation gate 16/16, no equivalent mutations; `git diff --check` clean; review patch 15 diffs. Record: checkpoint "C-4C". Scope as issued — catalog/V2 reconciliation: one strict-JSON capability constant in `questionnaire-v2-model.js` that the compiler reads; `event.work.period`, `event.education.period`, `event.separation.date`; activity → `event.activity.period`; behavioural parity test. Then C-4D: first the migration-runner atomicity/FK test and narrow repair, then the schema gaps already recorded (work / education / separation dates; the missing `activity` event type) — the next unused migration number, on scratch databases only, stack down | **D carries the WHOLE authority ledger, checkpoint B1–B17**, not a short list. The real-narrator identity floor stays an **open D-0 decision** |
>
