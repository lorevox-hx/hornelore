...
> | **C-4F** | **ACCEPTED / CLOSED 2026-09-26** (`2836a15` code, `fd6c4bf` record; ChatGPT ACCEPT after one pre-commit repair). Homes, unions and separations edited as Life Record occurrences on stable event ids; participant correction on the same event; new writer rule — derived relationships match their event's people (updated atomically); explicit current home (never inferred; several allowed); kind of home; stated marriage count; `event.separation.participant`; intake `current_residence` established in the Life Record; person-independence scenario; stale 0065 test repaired. **MAG-Chris `.venv`: 232 OK, 0 skips; catalog current; DESIGN COHERENT (22 · 14 · 23 · 11); 32/32 · 11/11 · 16/16.** Second-Keep behaviour carried as BACKLOG "QV2 pending-edit composition". Record: checkpoint "C-4F" |
> | **next** | **C-4R — controlled V2 catalog reset. STOP: no implementation until its work order is issued; R3-0 (how domain membership is stored) is a stop/go decision.** Life Record becomes the concept authority, old vocabularies become bindings; R5 cuts over only Life Record validation + V2 read model / capabilities / projections — extractor, Lori and memoir stay in Batch D. Then C-4G-0 → C-4G editors. Scope and sequence: checkpoint "C-4R" | **D carries the WHOLE authority ledger, checkpoint B1–B17**, not a short list. The real-narrator identity floor stays an **open D-0 decision** |
>
