...
2. **New lane: WO-EX-BINDING-01.** Address schema-binding failures exposed by Type C case_082 on **two orthogonal axes** surfaced by the per-item read. (a) **Routing layer:** residence vs personal.placeOfBirth (movement-value leaking to birthplace-scalar); residence.period vs education.gradeLevel (duration-anchor schema slot); sibling vs narrative references. (b) **Cardinality layer:** scalar fields (placeOfBirth, dateOfBirth, firstName, lastName, birthOrder) must emit ≤1 value per extraction call; excess candidates demote — either remap to the correct list-field if a routing rule identifies one, or drop with a log line. Smoke-test target: case_082 V1 and V6 both. Sequencing: BINDING-01 runs **after** SPANTAG (not alongside), to preserve clean causal attribution between span-separation and binding deltas. Spec: `WO-EX-BINDING-01_Spec.md`.
3. **Sequencing (SPANTAG → BINDING, not coupled).** SPANTAG first, alone — measures pure span-separation lift under r5f-spantag. BINDING-01 layers on top as a separate eval (r5l) under `HORNELORE_BINDING=1`. Coupling them into a single Pass 2 prompt would confound the eval gate; the matrix that produced this WO explicitly set up clean causal attribution and we will not collapse it now. Any Type C prompt help that must live inside SPANTAG Pass 2 lands as a SPANTAG addendum, not a BINDING deliverable.
4. **No successor "FIELD-DISCIPLINE-01" lane is minted or deferred.** Cardinality enforcement is folded into BINDING-01 Layer 2 (per the per-item read; the over-emission is prompt-invariant, so it's extractor-internal binding, not routing). If Layer 2 proves insufficient and a further domain-gating lane emerges, it will be named and scoped separately on its own evidence. Explicitly **not** FIELD-DISCIPLINE-01 — #142 owns "discipline."
