...
**Created:** 2026-06-15
**Harness:** `scripts/run_jake_long_narration_harness.py`
**Task:** #77

## Purpose

Stress-test the new oral-history + Phase 1 stack on a fresh
...
## OPERATOR PROTOCOL (manual paste-and-observe path)

The harness automates the eight steps below. This protocol stays in the
spec so a human operator can run the test by hand if the harness is
unavailable or if they want to score the subjective read in real time.
...
1. **Create a fresh narrator** named `Jake Max Miller`.
   ...
   The harness preloads the FULL intake payload (parents, siblings,
   spouse, children, military, faith, today) instead. This exercises
   the WO-OPERATOR-NEW-NARRATOR-INTAKE-FORM-01 endpoint end-to-end
   and tests Lori's behavior with a richly-seeded narrator. The "near-
   empty narrator" condition above describes the original manual
   variant — keep both in mind when interpreting results.
...
The harness runs these as part of its log-summary step and emits the
...
Note: when the harness preloads the intake payload, several of these
fields are already in `bio_facts` at `status='operator_entered'` before
Jake speaks a word. The extraction validation is therefore about
*chat-side extraction during the long-narration turns* — confirming the
extractor still fires correctly when the operator has pre-seeded a
substantial profile. A future variant of the harness with `testing_only=true`
and no intake payload would test the cold-narrator case.
...
The harness emits items 1, 2, and 3 mechanically as a report file at
`docs/reports/jake_long_narration_<conv_id>.txt`. Items 4 and 5 are
human judgment calls that Chris (or whoever ran the test) writes up
separately, citing the report file as the diagnostic anchor.
