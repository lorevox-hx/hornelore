...
| Portable Narrator Phase 6, desktop origin — three real narrators restored into clean roots and re-exported `EQUIVALENT` with no interaction; defect E (fresh root refused `db_not_found`, router now initialises the installation first); comparator: empty lane = absent lane; `questionnaire_put` bio_facts duplication proven not to be restore | [`wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md`](wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md) §36 · `launchers/hornelore_run_clean_root.sh` · BACKLOG §5 | — 2026-09-11; laptop origin next (§36.4) |
| Portable Narrator operator entry (Phase 5a) — Narrator Data Center card; two verdicts (integrity vs readiness); `0056` export-job ledger; artifacts and staging outside `DATA_DIR`; no verb deletes narrator data; 5b post-Phase-7 | [`wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md`](wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md) §35 · §35.1 · §35.2 · `server/code/api/routers/operator_narrator_package.py` · `tests/test_operator_narrator_package_api.py` | — Phase 5a code landed 2026-09-11; live acceptance walked and ACCEPTED the same day (§35.3): four repairs — picker switch timing, job re-attach on reload, dry-run dependency lookup on the referenced column not the PK, `init_db` seeds `chat_ws` |
| Portable Narrator round-trip equivalence — semantic comparator; installation-local integer-id boundary (`turns.id`) recorded | [`wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md`](wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md) §34 + Phase 4 finding · `tests/test_narrator_package_roundtrip.py` | — Phase 4 landed 2026-09-11 |
| Portable Narrator restore crash recovery — `db_committed` in-transaction, `0055` durable file manifest, hash-gated cleanup, package-independent verification | [`wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md`](wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md) §33.1 · `recover_restore_jobs()` | — 2026-09-11 |
| Portable Narrator dry-run + Restore v1 — files first, one deferred-FK transaction, ids verbatim, durable job `0054` | [`wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md`](wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md) §33 · `server/code/api/services/narrator_package.py` · `tests/test_narrator_package_restore.py` | — Phase 3 landed 2026-09-11 |
| Portable Narrator exporter — review corrections (§29.3 verified staging, byte binding, nested symlinks refuse) | [`wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md`](wo/WO-LOREVOX-PORTABLE-NARRATOR-01.md) §32 "Review corrections" | — 2026-09-11 |
