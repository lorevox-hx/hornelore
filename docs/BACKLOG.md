...
| Whole-tree `discover` isolation acceptance undemonstrated | `PLAN_2026-07-13` §A1 | the two named `DB_PATH` sites now save and restore, and `tests/__init__.py` sets a temp path — but A1's acceptance ("no spurious trips-table errors under full discover") has never been shown, and `CLAUDE.md` still mandates per-module runs. **Narrowed 2026-09-25 (C-4D):** one real cause found and repaired — `test_db_connection_hygiene` replacing the shared `api.db`, the package `db` binding and `DATA_DIR` (§12); targeted trips/hygiene isolation passes in both orders under `.venv`. Full discover still not run, so still not claimed |
...
- **Migration checksums.** `schema_migrations` records filenames only, so an already-applied migration that is later edited is never noticed (this is what happened to the draft `0063` on 2026-09-24). Add a content hash per applied file, and warn at startup when a recorded file's hash no longer matches. Until then: the `CLAUDE.md` immutability rule, plus `scripts/check_schema_drift.py`.

## 12. Carried from Batch C-4 — 2026-09-25 (none blocking the roadmap)

- **Pre-C-3 bare-string relationship periods** *(C-4F measured: unreachable from C-4F — it edits event periods, which are assertions, and never a relationship period; still open for whichever editor next edits relationship periods)*. A period end stored as a bare string (`"1961"` rather than `{text, value, precision}`) shows blank in the V2 relationship edit form and would be dropped if the operator re-kept that form. No such row is known: C-3 never wrote one, and the C-4 fixture corpus has no bare-string legacy case. Handle only if one is found; the fix belongs with the editor that next touches periods (C-4F).
- **~~C-4D test red since C-4E~~ — REPAIRED in C-4F (test-only, on review direction).** `test_migration_0065_activity_event.test_activity_is_admitted_every_old_type_still_works_and_bogus_is_refused` adds a `birth` for a narrator whose fixture already has one; C-4E's "one birth and one death per person" refuses it. The C-4E bank did not include this module. Repaired by writing each old type on its own fresh person and reading all twelve back; the rule and 0065 are unchanged. Checkpoint "C-4F".
- **Place `parts` has no contract** (C-4F). `lr_places.parts_json` is accepted as any JSON and read by nothing. A vocabulary (address · neighbourhood · city · county/region · state/province · country), operator-supplied only and never split from the label, is a schema decision owed before any editor writes it.
- **Childhood-home flag has no representation** (C-4F). The design names "childhood/current flags"; only `current` exists. Decide: a stated attribute, or none — never inferred from the period and the birth date.
- **Latent intake legacy family writes** (C-4F; owner C-7 / D-0). `/api/people/intake` still accepts family, spouses, `marital_status`, `number_of_marriages`, children and education, and writes split names, `Father`/`Mother` relation labels, list-length COUNTS and a marital-status scalar into `profile_json` / `bio_facts`, none into the Life Record. The shipped form sends none of it; reachable by direct API call only.
- **Lineage labels presume a parent's gender** (C-4F). "Mother's side" / "Father's side" over stored `maternal` / `paternal`. Infers nothing; wording only. A C-3-vocabulary decision.
- **QV2 pending-edit composition — OPEN** (found in the C-4F review, 2026-09-26). A second Keep against the same canonical subject before Save currently REPLACES the first pending edit, because the form rebases from stored state, not pending state. Seen in the C-4E vital forms and the C-4F occurrence forms. **Required eventual invariant:** multiple Keeps to one subject compose into one pending change set without dropping fields changed by an earlier Keep. **Do not solve separately per form — fix once in the shared pending-edit model.** Regression: stored occurrence (participant Anna, date 1980) → Keep #1 date 1982 → Keep #2 participant Maria → Save → the SAME occurrence id with participant Maria AND date 1982 (not 1980).
- **`questionnaire_schema.py:312` leaves a file handle open** (`open(...).read()`, a `ResourceWarning` in every bank run). Cleanup only; affects no result.
- **Background mutation runs in the agent sandbox.** A mutation gate started in the background from an agent sandbox is killed when the tool call returns, and `tests/mutation_runner.py`'s `finally` does not run on `SIGKILL` — so the live mutation survives. Seen once during C-4C (compiler line restored by hand, verified against every anchor). Run mutation gates in the foreground; under `.venv` on the laptop this does not arise. **C-4F addendum:** the host also cuts a FOREGROUND call at ~178 s regardless of the requested timeout, and a detached (`setsid nohup`) process dies with the call too — so from the sandbox, run ONE mutation per call.
- **~~Cross-suite trips isolation failure~~ — CONFIRMED AND REPAIRED in C-4D (2026-09-25).** Cause: `tests/test_db_connection_hygiene.py` deleted `api.db` from `sys.modules` at import and left a fresh copy installed; a suite loaded earlier kept the old module (and its `DB_PATH`), while trips code resolving `from .. import db` at call time got the new one. Reproduced under `.venv` by `unittest tests.test_c1_trips_person_id_fk_migration tests.test_db_connection_hygiene` (FAILED, errors=1). The hygiene suite now saves and restores `DATA_DIR`, every `api.db` / `*.api.db` module object and the packages' `db` attribute around its isolated import; pinned by `tests/test_db_hygiene_isolation.py` (DATA_DIR unset and set) and mutations H1–H3. `.venv`: trips → hygiene **16 OK** and hygiene → trips **16 OK** (FAILED before); the full C-4D same-process bank 179 OK; H1–H3 caught. **This does NOT prove whole-tree `discover` isolation** — see the §9 row below, narrowed, not closed.
