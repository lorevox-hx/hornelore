...
3. The narrator's **accepted DOB** is the start anchor of Hornelore's **existing seven-era life-span scaffold**, not of a new event spine. For a living narrator the span **stays open**, as the shipped scaffold already does; for a deceased narrator, a *known* death date bounds the span; a deceased narrator with an unknown death date is marked deceased and gets no invented endpoint (and never becomes today). *(D5(a), 2026-09-22. This item said the present-day boundary "is computed at render" until then; the product has never done that, per checkpoint §3.3b.)* Unknown DOB yields no fabricated scaffold; approximate DOB and derived ages retain uncertainty. Related family history may precede the narrator's birth. Typed occurrences/periods supply connections without replacing this scaffold.
