...
| | |
|---|---|
| **Live acceptance** | ⚠️ **PARTIAL 2026-09-07 — 6 passed / 0 failed / 3 unverified.** Run `guard-lab-20260907_124346`. Four steps closed live: atomic `All Switchable Off` (one revision, 37 overrides, protected untouched), a single-authority change (`onlyIdsChanged: [37]`), **two turns consuming revisions 1 and 2 in one process with no restart**, and `testing_only` persisting through the product creation path. **Under `All Switchable Off`, `raw_equals_delivered` is TRUE on both turns — zero words removed**, against Walt's 69%. Record: `docs/reports/GUARD-LAB-LIVE-ACCEPTANCE-20260907.md` (local-only) |
| **STILL OPEN — carried, not waived** | The mid-turn freeze OBSERVED (needs `--toggle-at`; the trace alone cannot decide it) · restart persistence (needs the `before-restart` / `after-restart` pair) · an ordinary narrator refused BY NAME with `not_testing_only` (use a synthetic `ZZ COHORT` narrator) · the stale second-tab 409 on a real browser. **`verify` exits 2 while any of these is unverified**, and that is the point |
| **Next slice** | The compact Guard Lab card on the main Operator tab — configuration (Defaults / Lean / Custom), revision, **whether the narrator being interviewed is eligible**, armed + recording, the two atomic buttons, a link into the Bug Panel. **Do not duplicate the 43-row table.** It closes the gap the live run exposed: the panel knows an eligible narrator exists, not whether you are talking to them |
