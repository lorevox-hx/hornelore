...
establish or change this concept today?* It no longer means *did the legacy questionnaire
schema have a field bound to it?* Nothing is declared for C-4E–G ahead of its control; each
later slice adds its concepts to the same declaration when its control lands. Amendment
(Chris, 2026-09-25): only an `also_known_as` name is `person.name.alias`; a `variant` is a
whole variant name, `person.name.full` only.

**Built:**

| Where | What |
|---|---|
| `ui/js/questionnaire-v2-model.js` | ONE capability object, strict JSON between `QV2_CAPABILITIES_JSON_BEGIN/END`; `isEditableConcept`; exported with `EVENT_DATE_CONCEPT` (now also separation, education, work, activity — read model only, no forms) |
| `ui/js/questionnaire-v2.js` | `can(concept)` gates every concept-writing control and its handler: answers, pronouns, names (edit/add), the also-known-as kind, preferred name, the add-someone form (kind, name, lineage, life status, period) and the relationship period fields. Undeclared → shown, not editable. An undeclared period is carried as stored, never dropped |
| `scripts/catalog/compile_concept_catalog.py` | reads the declaration (fail-closed on missing / duplicate markers, non-JSON, wrong shape, duplicates, non-strings, undefined or derived concepts); `questionnaire` = derived_readonly / editable-if-declared / not_offered; `store.EVENT_DATE_CONCEPT` is a measured concept binding (`bindings.event_dates`), each mapped concept must exist and be a date/date_interval. Provenance: `questionnaire` = declared QV2_CAPABILITIES; `legacy_questionnaire_paths` = measured questionnaire_schema (path membership, still on `bindings.paths[].in`). New fingerprint `qv2_capabilities_literal_sha256`; `counts.questionnaire` |
| `concept_catalog_source.py` | `event.work.period` (date_interval, many), `event.education.period` (date_interval, many), `event.separation.date` (date, one) — no form path invented |
| `life_record/store.py` | `EVENT_DATE_CONCEPT` + separation, education, work, activity. The writer's placement rule now admits those dates on their own event types. **No migration:** the `lr_events.type` CHECK still refuses `activity` (C-4D) |
| `concept_catalog_v1.json` | regenerated by the compiler only |

**The editable set (12), measured by driving the shipped controls:** `person.name.full`,
`person.name.preferred`, `person.name.alias`, `person.life_status`, `person.pronouns`,
`person.birth.order`, `person.reported_count.siblings`, `…children`, `…grandchildren`,
`relationship.kind`, `relationship.period`, `relationship.qualifier.lineage_side`.

**Catalog before → after:** editable 61 → **12**, not_offered 23 → **75**, derived_readonly
1 → 1; concepts 85 → 88. Became editable: pronouns, the three counts, `relationship.period`,
`person.name.alias`. Stopped being editable (the old form offered them; V2 has no control):
given/family/birth-family name parts, birth/death/union/residence/service/work/education/
activity/retirement concepts, faith, heritage, interests, occupation, animals, stories, trips.

**Tests.** `tests/test_qv2_capabilities.py` + `tests/qv2_capability_harness.js`: the
harness drives every shipped V2 control against the real writer (via `qv2_bridge.py`); the
Python side translates the accepted PATCH into concepts by the structural rules in its
docstring and requires **actual == FROZEN_EXPECTED (written in the test) == declaration ==
compiled `editable`**, with no untranslated write; the relationship-period *edit* is
translated on its own; names stay whole and only `also_known_as` is an alias; read-only
and future concepts are not editable; with the declaration emptied at runtime the editor
offers no concept-writing control (fail closed); the compiler's parser refuses every
malformation; browser and server event-date maps are identical; the committed catalog
records the old form's paths while calling them not_offered. `tests.test_life_record_writer.
EventDatesC4C`: work/education/separation dates land on their events, the wrong concept is
refused on each, `activity` is mapped but refused by the schema. `test_qv2_adapter`: the
"work carries no date concept yet" assertion now reads `event.work.period`, blank.

**Mutations.** `tests/mutate_qv2_capabilities.py` (new, 16, in place via
`tests/mutation_runner.py`; suite = the capability tests + `EventDatesC4C` + `compile --check`):
**16/16 caught** in the sandbox. Two are marked `[oracle]` — they edit the test's translator,
not product code, to prove the frozen set pins the structural translation. One (the
relationship-period edit translation) first SURVIVED because the added relationship also
carried a period; the test now translates the edit on its own, and it is caught. No
equivalent mutations. `scripts/catalog/mutate_concept_catalog.py`: its minimal tree now
includes `questionnaire-v2-model.js` (the compiler reads it); **28/28 caught**.

**Acceptance evidence — `.venv`, Chris's laptop, 2026-09-25:**
- focused bank (`test_concept_catalog`, `test_qv2_capabilities`, `test_qv2_people`,
  `test_qv2_adapter`, `test_life_record_writer`, `test_life_record_dates`,
  `test_life_record_schema_contract`): **150 tests OK, 0 skips**;
- `compile_concept_catalog.py --check`: **catalog is current**;
- design validator: 20 rules · 14 situations · 22 refusals · 11 contracts — **DESIGN COHERENT**;
- catalog mutation gate (`scripts/catalog/mutate_concept_catalog.py`): **28/28 — every mutation caught**;
- QV2 capability mutation gate (`tests/mutate_qv2_capabilities.py`): **16/16 caught**, no
  equivalent mutations;
- pre-commit: `git diff --check` clean; complete review patch **15 diffs**.
...
`test_qv2_shell` OK; `compile --check` current; design validator COHERENT.
