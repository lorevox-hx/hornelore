...
|---|---|---|---|---|
| 1 | `_overwriteBbPersonal` | `session-loop.js:749` PUT, `source: bug227_identity_rescue` | a **heuristic on narrator chat text** (`_hasIdentitySignal`, `:290`) | the only browser path that **overwrites** non-empty `fullName / dateOfBirth / placeOfBirth`, no confirmation, no undo. **`inferred`: a plausible mechanism for how Janice's record acquired a wrong birth date and lost a middle name.** Not verified against her history |
