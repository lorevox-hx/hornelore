...
**LOOP-01 R5.5 Phase 1 — WO-EX-NARRATIVE-FIELD-01 Phase 4 eval pending (r5e).** R4 is closed. r4i is the locked post-R4 baseline (see `docs/reports/POST-R4-BASELINE-LOCK.md`). SPANTAG implementation paused behind SECTION-EFFECT-01 Phase 3 (#95).

NARRATIVE-FIELD-01 Phases 1–3 landed 2026-04-20 → 2026-04-21 (5 commits stacked since r5d): Phase 2 narrative fewshots + anti-consolidation + narrator-identity priority (`3fec639`, `9706a87`), CLAIMS-02 role-exempt (`291cbb8`), Phase 3 Fix A scorer alias (`77e5b52`, measurement-side), Phase 3 Fix C date-range preference fewshot (`74f7b0d`). r5e is the composite measurement. Smoke evidence going in: case_049 = 1.000 (clean fix); case_040 deterministic pre-role-exempt, should green role field post; winnerguard 4/5 → 5/5 expected. Floor r5d = 58/104 v2=30. Prediction 59–62/104 v2=31–33. Scorer-drift audit required on every flip because Fix A lives in the eval script.

Active sequence:

1. **#116 / NARRATIVE-FIELD Phase 4** (IN PROGRESS) — master r5e measurement + decision gate. Sequence blocks #117 final report.
2. **#63 / WO-EX-SECTION-EFFECT-01 Phase 3** (#95) — 2–3-case causal matrix varying stage context. Phases 1 & 2 landed. Unblocks SPANTAG.
3. **#119 / turnscope greatGrandparents fix** — small surgical fix to `allowed_branches` when section targets greatGrandparents subtree.
4. **#90 / WO-EX-SPANTAG-01** — two-pass extraction: Pass 1 evidence-only NL tag inventory, Pass 2 bind/project with **section / target_path / era / pass / mode as explicit controlled priors** (not implicit forces). Subject-beats-section rule when Pass 1 binds a non-narrator relation_cue. Dual-path emission as first-class Pass 2 output. First target pack = the 15 stubborn cases, scored against SECTION-EFFECT-01-adjudicated labels. Flag `HORNELORE_SPANTAG=1`, off by default. **Implementation unblocks after SECTION-EFFECT-01 Phase 3 signoff.** See `WO-EX-SPANTAG-01_Spec.md`.
5. **Post-SPANTAG gate** — if SPANTAG ships default-on, R5.5 Pillar 2 (entity-role binding) opens. If it doesn't, iterate SPANTAG or revisit.
